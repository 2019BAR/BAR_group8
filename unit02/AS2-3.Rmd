...
```{r}
A = read.csv('C:/Users/user/Desktop/R/data and code/unit02cd/other data/CPSData.csv')
MetroAreaCode = read.csv('C:/Users/user/Desktop/R/data and code/unit02cd/other data/MetroAreaCodes.csv')
CountryOfBirthcode = read.csv('C:/Users/user/Desktop/R/data and code/unit02cd/other data/CountryCodes.csv')
nrow(A)
```
...
```{r}
table(A$Citizenship) %>% prop.table %>% round(3)
```
...
```
+2.2 married variable missing is related to the Age value for the interviewee.
...
```{r}
tapply(is.na(A$MetroAreaCode),A$State,mean) %>% sort %>% head
```
...
```{r}
(tapply(is.na(A$MetroAreaCode),A$State,mean) %>% round(3) -0.3 %>% sort)
...
```{r}
tapply(A$Race == "Asian", A$MetroArea.y, mean) %>% sort %>% tail
```
...
```{r}
tapply(A$Education == "No high school diploma", A$MetroArea.y, mean, na.rm = TRUE) %>% sort %>% head
```
...
```{r}
table(A$Country[A$MetroArea.y == "New York-Northern New Jersey-Long Island, NY-NJ-PA"] =="United States") %>% prop.table
```
...
```{r}
tapply(A$Country =='India', A$MetroArea.y, sum) %>% sort %>% tail
``` 

In Brazil?
```{r}
tapply(A$Country =='Brazil', A$MetroArea.y, sum) %>% sort %>% tail
```

In Somalia?
```{r}
tapply(A$Country =='Somalia', A$MetroArea.y, sum) %>% sort %>% tail
```
